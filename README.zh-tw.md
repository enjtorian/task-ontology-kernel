...
TOK 定義任務的本體結構身份、狀態、生命週期、依賴關係與演化方式使任務成為可被 AI 原生執行、版本化、治理與演化的系統級 Primitive，開啟 **Task-Native** 軟體工程範式。
...
*   **Task Ontology Kernel (TOK)**：定義任務的本體結構任務是什麼、如何存在、如何演化。
*   **TOCA（任務導向認知架構）**：以任務為核心的閉環認知系統捕獲、調度、執行、驗證、演化。
*   **Task Object 四層結構**：Intent、Context、Strategy、Evaluation任務的通用解剖結構。
...
> 在反覆使用 LLM 的過程中，我察覺到一個根本性的轉變：Prompt 不再是一次性的指令，而是可重用的認知結構。這一靈光一閃揭示了一個更深層的真理任務正在從人類的心理單位，演化為系統的執行 Primitive。
