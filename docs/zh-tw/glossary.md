...
定義任務本體結構的理論基礎——包括身份、狀態、生命週期、依賴關係與演化方式。TOK 將任務從非結構化的心理概念轉化為可被 AI 原生執行、版本化、治理與演化的系統級 Primitive。它回答：什麼是任務、任務如何存在、任務如何演化。
...
### Intent Layer（意圖層）
Task Object 的第一層。描述**目標狀態**——應該達成什麼——而非執行步驟。必須語義清晰，能被 LLM 解譯為判斷邏輯。
...
### Agent-Native 執行
一種執行模式，任務不是直接作為腳本執行，而是**委託給 Agent**，由 Agent 自主決定執行方式——選擇工具、使用 LLM 推理、呼叫 API，或建立新任務。
...
TOK 的分類定位——模型化「世界將如何改變」的可執行意圖——相對於 Palantir 的 **Object Ontology**（物件本體），後者模型化「世界中有什麼」。
